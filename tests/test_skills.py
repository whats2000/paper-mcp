...
async def test_skills_are_served_as_mcp_prompts() -> None:
    ...
    assert {"deep-read"} <= {p.name for p in prompts}
    assert all(p.description for p in prompts)


def test_the_read_skill_states_the_grounding_contract() -> None:
    # The one thing this skill exists to prevent is an agent citing a figure
    # the paper does not contain.
    body = load_skills()["deep-read"][1]
